...
#: Statuses meaning the last real Turn Gemini send did not succeed. The
#: standalone send control is a permanent-normal-flow control no longer --
...
class BattleRecordUiWindow(TurnSnapshotMatchFlowWindow):
    ...
    def _restructure_battle_record_layout(self) -> None:
        ...
        # Tournament-week: CONFIRM TURN FACTS persists the reviewed facts
        # and, once that leaves the binding provider-ready, also sends
        # exactly one Turn Gemini request itself (see
        # _on_confirm_turn_facts) -- the standalone button below is kept
        # only as a hidden-until-failure retry, not a required second step.
        ...
        if gemini_send_button is not None:
            # Reuse this pre-existing, already-correctly-wired (via the
            # inherited ``_on_trusted_send_turn_to_gemini`` override ->
            # ``send_rich_turn_advice_to_gemini``) explicit send control
            # instead of inventing a new one. render_view() now keeps it
            # hidden during normal operation (CONFIRM TURN FACTS already
            # sends) and only reveals it, relabeled, as a failure retry --
            # see the _TURN_ADVICE_FAILURE_STATUSES check there. Placed in
            # the rich-state status group it already reports readiness/
            # denial reasons for.
    ...
    def render_view(self, view: OperatorView | None = None) -> None:
        ...
        if gemini_button is not None:
            # Tournament-week: CONFIRM TURN FACTS is now the operator's one
            # normal send action (it sends itself once provider-ready --
            # see _on_confirm_turn_facts). This control is therefore no
            # longer a permanent second send path: it stays hidden during
            # normal operation and reappears, relabeled as an explicit
            # retry, only when the last real send actually failed. Its own
            # click handler/dispatch/duplicate-in-flight guard/binding
            # validation are unchanged either way.
            failed_last_send = status.status in _TURN_ADVICE_FAILURE_STATUSES
            show_retry = (
                projection.primary_cta == "REQUEST_TURN_ADVICE" and failed_last_send
            )
            gemini_button.setText("Gemini再送信" if failed_last_send else "SEND TURN TO GEMINI")
            gemini_button.setVisible(show_retry)
    ...
    def _on_confirm_turn_facts(self, _checked: bool = False) -> None:
        ...
        self.render_view(view)
        # Tournament-week change: CONFIRM TURN FACTS is now the operator's
        # single explicit action for both confirming facts and sending Turn
        # Advice. Confirming always persists the facts regardless of
        # provider readiness; only when the freshly confirmed revision is
        # provider-ready does this same press also trigger exactly one Turn
        # Gemini send, by re-invoking the standalone send button's own
        # handler. render_view() above already reflects the just-persisted
        # state, so turn_state_summary() here reads the post-confirm
        # revision, never a stale pre-confirm one. Re-entrancy/double-send
        # protection is not duplicated here: GeminiRichTurnAdviceAdapter.send()
        # already refuses a second concurrent dispatch while one is in
        # flight, and this call site is reachable only from a trusted human
        # button press (never render/refresh/OCR/timers), so that existing
        # guard is sufficient. The standalone SEND TURN TO GEMINI button
        # remains as a manual fallback/retry for when this combined send
        # does not fire (provider not ready yet) or fails.
        if self._bundle_c_controller.turn_state_summary().provider_ready:
